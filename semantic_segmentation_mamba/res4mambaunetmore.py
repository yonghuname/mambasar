...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        attn_output = self.norm(attn_output + x)  # 残差连接与归一化
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        x=self.mha(x)
        x= self.dab(x)
        attn_output = x.permute(0, 3, 1, 2).contiguous()
        print(f"xsize",x.size())
...
class res4deepMambaunetv2(nn.Module):
    def __init__(
            self,
...
class res4deepMambaunetwithmultihead(nn.Module):
    def __init__(
            self,
            patch_size=4,  # 补丁大小，表示图像分块的大小
            in_chans=4,  # 输入通道数，这里为4，表示输入图像有4个通道
